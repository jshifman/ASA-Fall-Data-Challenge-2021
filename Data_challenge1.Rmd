...
```{r }
my_us_grid <- us_state_grid1[c(-2, -8), ] # remove AZ, DE  # fina a way to remove DC & SD 

plot_low10 <- ggplot(df_clean, aes(Race, Percent, fill = Race)) +
  geom_col() +
  coord_flip() +
  theme_bw() + 
  facet_geo(~ State ,grid = my_us_grid) + theme(text = element_text(size = 6)) 
...
print(plot_low10 + mynamestheme + labs( title = "Percent Distribution of Race Throughout Different States For \nLow Access Tract That is 10 Miles Away From Supermarket") )
```




Testing the facet_geo 
code from 
