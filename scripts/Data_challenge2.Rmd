...
```{r}
suppressMessages(allStatesDataFrame <- read_xlsx("~/Desktop/Dataset_ Food ASAFDC 2021.xlsx", 3))
...
```{r}
plot_bar <- data_new_tidy_bar %>% ggplot(aes(Race, Percent, fill = Race,pos)) + geom_col() + scale_x_discrete(guide = guide_axis(n.dodge=2)) + ylab("Sum of Percentage for Each State") 

```

```{r}
print(plot_bar + mynamestheme + labs( title = "Sum of Percentage for Each State for Various Race Groups That is 10 Miles Away From Supermarket") ) 
#+ ggsave("ggplot_with_overlapping_axis_text_wide.png", width=8, height = 4)
```
